...
class UpdateWalletBizSuit extends StatefulWidget {
  static String tag = '/UpdateWalletBizSuit';
  String name;
  int thetype;
  String thebnkaccno;
  String thebankname;
  ...
  UpdateWalletBizSuit({Key key,@required this.routeArgument }) {
    name= this.routeArgument.argumentsList[0] as String;
    thetype=this.routeArgument.argumentsList[1] as int;
    thebnkaccno=this.routeArgument.argumentsList[2] as String;
    thebankname=this.routeArgument.argumentsList[3] as String;
    thecolorcode=this.routeArgument.argumentsList[4] as String;
  }

  @override
  UpdateWalletBizSuitState createState() => UpdateWalletBizSuitState();
}

class UpdateWalletBizSuitState extends State<UpdateWalletBizSuit> {
...
cat=1;
                               ...
                               addwallet(name:_name.text,category:cat,newbankornot:newbankornot,bankacc:_selectedLocation);
                        },
                      ),
                    ),
                    SizedBox(
        ...
        if (!allbanks.contains(joinnameacc)) {
          setState(() {
            allbanks.add(joinnameacc);
                  ...
                  if(widget.thebnkaccno==getbanks.banks[i].bankaccno){
              _selectedLocation = "${getbanks.banks[i].bankaccno}/${getbanks.banks[i].bankname}";
            }
          });
        }
         if (_selectedLocation == null ||
                ...
                _selectedLocation == ''||
                _selectedLocation == ' ') {
              _selectedLocation = "${getbanks.banks[0].bankaccno}/${getbanks.banks[0].bankname}";

                }
      }
      }
                          if(widget.thecolorcode=="3"  ){
 setState(() {
   _selectedlabel="Blue Label";
 });
                          }else if(widget.thecolorcode=="2"){
 setState(() {
  _selectedlabel="Orange Label";
 });

                          }else if(widget.thecolorcode=="1"){
 setState(() {
_selectedlabel="Green Label";
 });
                          }  
 ...
 addwallet({ name,category,newbankornot,bankacc}) async {
    functionfile.onHorizontalLoading1(context);
    var theinternet = await functionfile.isInternet();
    var theinternet2 = await functionfile.checkInternetAccess();
    ...
    try {
      if (theinternet && theinternet2) {
        if (name.isEmpty) {
          ...
          var v = await bizsuitapi.userupdatewallet( name:name,category:category,bankselectedid:bankselectedid,wallid:widget.thetype)
          ...
          if (v != null) {
            if (v.status == true) {
              Navigator.of(context).pop();
              toast("Great. Wallet updated Successfuly!!!",
                  gravity: ToastGravity.CENTER,
                  bgColor: AppColors.appDark_parrot_green);
