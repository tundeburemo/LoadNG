...
class SellUniversal extends StatefulWidget {
  static var tag = "/selluniversal";
String name;
String image;
String rate;
int thecolors;
int thetype;
 final RouteArgument routeArgument;
  ...
  SellUniversal({Key key,@required this.routeArgument }) {
  ...
  @override
  SellUniversalState createState() => SellUniversalState();
}

class SellUniversalState extends State<SellUniversal> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        text("Below Address can only be used once.",maxLine: 5,isCentered: true,fontSize: AppFontSize.textSizeMedium,fontFamily: AppFontSize.spinnaker).paddingTop(AppFontSize.spacing_control),
...
Future<void> userInfo() async {
    ...
    var getrate;
    if(mounted){
    var mytype= widget.thetype;
    if(mytype==0){
      getrate = await sellapi.sellblockc();
      // blockchain
    }else if(mytype==1){
      // universal
      getrate = await sellapi.sellunibc();
    }else if(mytype==2){
      // eth
      getrate = await sellapi.selleth();
    }else if(mytype==3){
      // ltc
      getrate = await sellapi.sellltc();
    }else if(mytype==4){
      // bch
      getrate = await sellapi.sellbch();
    }else if(mytype==5){
      // usdc
      getrate = await sellapi.sellusdc();
    }else if(mytype==6){
      // usdc
      getrate = await sellapi.selldai();
    }else if(mytype==7){
      // usdc
      getrate = await sellapi.selldoge();
    }
