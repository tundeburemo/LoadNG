...
class SellPm extends StatefulWidget {
  static var tag = "/sellpm";
  String name;
  String image;
  String rate; 
  int thecolor;
  ...
  SellPm({Key key, @required this.routeArgument}) {
  ...
  @override
  SellPmState createState() => SellPmState();
}

class SellPmState extends State<SellPm> {
                            ...
                            child: Container(
                              ...
                              child: Column(
                                ...
                                children: <Widget>[
                                  ...
                                  formField(context, "USD VALUE",
                                      isPassword: false,
                                      controller: _usdval,
                                      prefixIcon: Icons.attach_money, ontap: () {
                                    if (_usdval.text.isNotEmpty &&
                                        _usdval.text != ' ' &&
                                        _usdval.text != '') {
                                      _nairaval.text =
                                          (double.parse(widget.rate) *
                                                  double.parse(_usdval.text))
                                              .toString();
                                    }
    ...
    for (i = 0; i < thetotal; i++) {
      if (!allbanks.contains(getbanks.banks[i].bankaccno)) {
        if (mounted) {
          setState(() {
            allbanks.add(
                "${getbanks.banks[i].bankaccno}/${getbanks.banks[i].bankname}");
          });
        }
      }
    }
    ...
    try {
      if (theinternet && theinternet2) {
        if (usdval.isEmpty) {
          ...
          Navigator.of(context).pop();
          toast('Please choose a bank account',
              gravity: ToastGravity.CENTER, bgColor: AppColors.appLightRed);
        } else {
          ...
          if (getbanks != null) {
            ...
            var v = await sellapi
                .sellpm(amountselling: usdval, bankid: sendtbnkid)
            ...
            if (v != null) {
              if (v.status == true) {
                Navigator.of(context).pop();
                toast(
                    "Transaction Initiated Successfully, kindly wait while perfect money  loads",
                    gravity: ToastGravity.CENTER,
                    bgColor: AppColors.appDark_parrot_green);
                    Uri theurl=Uri.parse("${v.selllink}");
                Navigator.of(context).pushNamed('/inappwebviewpm',
            ...
            Navigator.of(context).pop();
            toast('Please Check your Internet Speed and retry',
                gravity: ToastGravity.CENTER, bgColor: AppColors.appLightRed);
          }
        }
      } else {
