...
class UserDashboardState extends State<UserDashboard> {
  ...
  void showHelloWorld() {
    showDialog(
      ...
      builder: (context) => new AlertDialog(
        ...
        content: Text(
            'You are on the old app version of LoadNG app, there is a new version of the app available on the store, kindly download the new app to stay updated with the new features we just added.'),
        actions: <Widget>[
          FlatButton(
            onPressed: () {
  ...
  Widget build(BuildContext context) {
    ...
    var sliderHeight =height<=600? height * 0.3:height * 0.28;
  ...
  text:  TextSpan(
    ...
    children: <TextSpan>[
    TextSpan(text: 'Introducing the LoadNG Points System.\n', style: TextStyle(fontWeight: FontWeight.bold)),
    TextSpan(text: 'The LoadNG Points System has from October 1, 2021 replaced the LoadNG Bonus Coin System. This was inspired by #CustomerAppreciationWeek\n\n',),
    TextSpan(text: 'About LoadNG Points (LP)\n', style: TextStyle(fontWeight: FontWeight.bold)),
    TextSpan(text: 'LoadNG Point is a bonus from LoadNG pegged at the rate of 10 Lp to 1 naira. This means 10000 points is equal 1000 naira.\n\n\n',),

        TextSpan(text: 'How to get LP\n', style: TextStyle(fontWeight: FontWeight.bold)),
    TextSpan(text: 'For every transaction on LoadNG, you are rewarded with LoadNG Points. You can see this in the notification tab on web or app.\n\nFor every active user you refer to LoadNG through your LoadNG referral link you will also get rewarded with LoadNG Points\n\n\n',),

            TextSpan(text: 'LP Withdrawal Terms\n', style: TextStyle(fontWeight: FontWeight.bold)),
    TextSpan(text: '1. You can only withdraw a minimum of 500 naira (5000LP).\n\n',),
    TextSpan(text: '2. LoadNG Points withdrawal is capped at 5000 Naira (50000 LP), your LP balance also resets automatically after every LP withdrawal.\n\n',),
    TextSpan(text: '3. You can only make a LP withdrawal once in 30 days (once a month), that means if you made a points withdrawal on 13th october, you have to wait till 13th November before your next point\'s withdrawal.\n\n',),

         TextSpan(text: 'This means that you can now withdraw your LoadNG Points anyday with respect to the terms above. We also now have a dedicated page on web (www.load.ng) and app (Android and iOS) for the new points system so you can keep track of all your LP earnings and withdrawals.\n\n',),
                  TextSpan(text: 'So keep earning... and thanks for using LoadNG.',),
                      ...
                      var thehexcode = 0xfffff0dd;
                      var theimage = AppImage.t12_card;
                      showDialog(
                          context: context,
                          builder: (BuildContext context) => CustomInfo(
                              sentimg: theimage,
                            ...
                            builder: (context) => new AlertDialog(
                              title: Text('FILL KYC',
                                  style: TextStyle(
                                      color: Colors.black, fontSize: 20.0)),
                              content: Text(
                                  'To buy coins on LoadNG, you have to fill in your KYC details and have them approved, kindly do so by clicking the button below. '),
                              actions: <Widget>[
                                Material(
                                    color: AppColors.shadowdarkblack2,
                                    child: FlatButton(
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    ...
                    isCentered:true,
                            ...
                            fontSize: AppFontSize.textSizeSmallest,
        ...
        child: Scaffold(
          ...
          body: NestedScrollView(
            headerSliverBuilder:
              ...
              return <Widget>[
                SliverAppBar(
                    expandedHeight: sliderHeight + 118,
                    floating: true,
  ...
  Future<void> userInfo() async {
      ...
      functionfile.onHorizontalLoading1(context);
       ...
       var checkeing62 = await systemapi.checkmaintanance();
      if (checkeing62 != null) {
      if (checkeing62.status) {
      Navigator.pop(context);

Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (ctx) => MainTananceScreen()));

      Navigator.of(context).pushNamed('/maintanance');
      ...
      style: TextStyle(color: Colors.black, fontSize: 20.0)),
      content: Text(
      'Sorry your account has been banned and you are not allowed to use any of our services again'),
      actions: <Widget>[
      Material(
      color: AppColors.shadowdarkblack2,
      child: FlatButton(
...
if(mounted){
       ...
       showDialog(
          ...
          builder: (context) => new AlertDialog(
            title:  Text('Internet Error',
                style:  TextStyle(color: Colors.black, fontSize: 20.0)),
            content:  Text(
                'Oops! Kindly check your internet connection as the data for the dashboard failed to load. Reload page to try again.'),
            actions: <Widget>[
               Row(
                                        crossAxisAlignment:
  ...
  //   print('Message also contained a notification: ${event.notification.android}');
